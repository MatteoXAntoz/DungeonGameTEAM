...
public class TeleportSkill extends Skill {
    ...
    public void teleport(Entity from, Point to) {
        if (from.getComponent(PositionComponent.class).isPresent()) {
            PositionComponent positionComponent =
                    (PositionComponent) from.getComponent(PositionComponent.class).get();
    ...
    public void teleport(Entity from, Entity to) {
        if (from.getComponent(PositionComponent.class).isPresent()
                && to.getComponent(PositionComponent.class).isPresent()) {
            PositionComponent positionComponentFrom =
                    (PositionComponent) from.getComponent(PositionComponent.class).get();
            PositionComponent positionComponentTo =
                    (PositionComponent) from.getComponent(PositionComponent.class).get();
